...
bestFitFile = '../rankNum/rankNum_coverfrac_actualBestFit_gauss.txt'
...
for run in runList:
    ...
    for i in range(len(ionList)):
        ...
        for v in range(4):
            plotvel = np.arange(min(obsvel), max(obsvel), 0.1)

            tau_coef = (c_km*singleIon[v][5])/(np.sqrt(np.pi)*singleIon[v][1])*np.exp(-(plotvel + singleIon[v][0])**2/(singleIon[v][1])**2)
            Flux_a = (1 - runInfo[v][1]) + runInfo[v][1]*np.exp(-1*tau_coef*singleIon[v][4])

            fitvel.append(-singleIon[v][0])
            estFlux_a.append(Flux_a)


            ax[i].plot(obsvel, obsflux, color = 'black')
            #ax[i].scatter(fitvel, estFlux_a, color = 'red')
            ax[i].plot(plotvel, estFlux_a[v], color = 'red')
    ...
    fig.savefig('coverfracApp/coverfracApp_'+run['Name']+'_all.pdf')
