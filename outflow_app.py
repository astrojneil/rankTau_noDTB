...
### dictionaries of ion info
    ...
    otherwrite = open('../rankTau/ranktau_outflow_actualBestFit.txt', 'w')
    otherwrite.write('RunName, vel, alpha, beta\n')

    alphas = np.logspace(-4, 3, 10000)
    betas = np.logspace(-4, 3, 10000)

    for run in runList:
        #save a lot of info
        runMultiCloud = np.zeros((len(ionList), 4))  #run[row][col] = run[ion][vel]
        runSingleCloud = np.zeros((len(ionList), 4))
        ionInfo = np.zeros((len(ionList), 4, 4))  #ion[ion][vel][param]
        obsFlux = np.zeros((len(ionList), 4, 2)) #obs[ion][vel][flux/noise]
        for i in range(len(ionList)):
            #open ionfile, find the velocities=0 tmax=1 q=2 and area=3
        ...
        for v in range(4):
            ...
            for beta in betas:
                ...
                for i in range(len(ionList)):
                    ...
                    def modelFun(x):
                        return np.exp(-beta*model(ionInfo[i][v][1], ionInfo[i][v][2], x))
                    ...
                    chi = (obsFlux[i][v][0] - estFlux)**2/obsFlux[i][v][1]
                    sumChib = sumChib + chi
            ...
            for alpha in alphas:
                ...
                for i in range(len(ionList)):
                    #calculate the estimated flux with this beta
                    estFlux = np.exp(-alpha*ionInfo[i][v][3])
            ...
            if run['Name'][-5:] !='_cond':
                ...
                betaWrite = '--'
            #print/save results
            otherwrite.write(run['Name']+', '+str(ionInfo[0][v][0])+', '+str(alphas[minChia_index])+', '+str(betaWrite)+'\n')
